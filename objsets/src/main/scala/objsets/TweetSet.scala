...
class Empty extends TweetSet {
  ...
  def mostRetweeted: Tweet = throw new java.util.NoSuchElementException("No such element in an empty set")
...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
  ...
  private def max (set:TweetSet, cur:Tweet):Tweet={
    if (!set.isEmpty)
    {
      var x:Tweet = set.mostRetweeted
      if (x.retweets > cur.retweets) 
        return x
    }
    cur
  }
  ...
  override def toString = elem.retweets.toString()
