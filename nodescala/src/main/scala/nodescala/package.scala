...
package object nodescala {
  ...
  implicit class FutureCompanionOps(val f: Future.type) extends AnyVal {
    ...
    def never[T]: Future[T] = {
      ...
      Future{p.failure(new TimeoutException)}      
    ...
    def all[T](fs: List[Future[T]]): Future[List[T]] = {
      ...
      fs.foldRight(successful.future) {(f,acc) => for{x <-f; xs <-acc} yield x::xs}
    }
    ...
    def any[T](fs: List[Future[T]]): Future[T] = {
      ...
      for(fut <- fs)
        fut.onComplete {p.tryComplete(_)}
  ...
  implicit class FutureOps[T](val f: Future[T]) extends AnyVal {
    ...
    def continueWith[S](cont: Future[T] => S): Future[S] = {
      ...
      f.onComplete {
        case x => p.complete(x)}
      //f onFailure {case t => p.complete(t)}
      Future {cont(p.future)}      
      
      /*val p = Promise[S]()
      
      f.onComplete {
        case Failure(x) => println("fdsfds");p.tryFailure(x)//p.complete(x)
        case Success(v) =>println("here");
          p.complete(Try{cont(Future(v))})
      }
      p.future*/
